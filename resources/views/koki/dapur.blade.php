...
@section('title', 'Monitor Dapur (Koki)')
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-400">Daftar item pesanan makanan yang ditugaskan kepada <span class="text-amber-400 font-semibold">{{ auth()->user()->username }}</span></p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-slate-800 border {{ $item->status_item === 'menunggu' ? 'border-amber-500/50 shadow-amber-500/5' : 'border-blue-500/50 shadow-blue-500/5' }} rounded-xl p-5 shadow-xl flex flex-col justify-between space-y-4">
                <div class="space-y-2">
                    ...
                    <div class="py-2">
                        ...
                        <p class="text-2xl font-extrabold text-amber-400 mt-1">{{ $item->jumlah }} <span class="text-xs font-normal text-slate-400">porsi</span></p>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-700/80 pt-3 flex items-center justify-between">
                    ...
                    <form method="POST" action="/dapur/detail/{{ $item->id_detail_pesanan }}/status">
                        @csrf
                        @method('PATCH')
                        @if($item->status_item === 'menunggu')
        ...
        @empty
            <div class="col-span-full bg-slate-800/40 border border-slate-700/50 rounded-xl p-12 text-center text-slate-500">
                ...
                <p class="text-sm font-medium">Tidak ada tugas pesanan makanan aktif untuk Anda saat ini.</p>
                <p class="text-xs text-slate-600 mt-1">Semua pesanan makanan telah selesai disiapkan!</p>
            </div>
        @endforelse
    </div>
</div>
